...
Alter table PATIENT 
	...
	-- CONTRAINTE DU BOOLEAN EXCLUSIVE
Alter table PATIENT 
...
CREATE OR REPLACE TRIGGER ANA_S???
BEFORE INSERT ON ANALYSESANG
FOR EACH ROW
BEGIN
	IF :NEW.PLAQUETTES>1000000 OR :NEW.PLAQUETTES<5000 THEN
		RAISE_APPLICATION_ERROR(-200010,'VALEUR PLAQUETTES ABERANTES');
	END IF;
	IF :NEW.POTASSIUM<0.1 OR :NEW.POTASSIUM>10 THEN
		RAISE_APPLICATION_ERROR(-200011,'VALEUR POTASSIUM ABERANTES');
	END IF;
	IF :NEW.SODIUM>200 OR :NEW.SODIUM<100 THEN
		RAISE_APPLICATION_ERROR(-200012,'VALEUR SODIUM ABERANTES');
	END IF;
	IF :NEW.PROTIDEMIE<10 OR :NEW.PROTIDEMIE>150 THEN
		RAISE_APPLICATION_ERROR(-200013,'VALEUR PROTIDEMIE ABERANTES');
	END IF;
	IF :NEW.CALCIUM<0.5 OR :NEW.CALCIUM>6 THEN
		RAISE_APPLICATION_ERROR(-200014,'VALEUR CALCIUM ABERANTES');
	END IF;
END;
...
CREATE OR REPLACE TRIGGER ANA_AUTO_METIER
...
DECLARE	
	...
	RISQUE INTEGER;
BEGIN
	SELECT PAT_IDPAT INTO NUMPAT FROM FICHE WHERE ELECTRO_ID_FICHE=:NEW.IDFICHE;
	SELECT RESULTATELECTRO INTO RES FROM ELECTROENCEPHALOGRAMME WHERE ELECTRO_ID_FICHE=:NEW.IDFICHE;
	SELECT RESULTATELECTRO INTO RESPRECEDENT FROM ELECTROENCEPHALOGRAMME WHERE EXISTS(SELECT DATEFICHE FROM FICHE WHERE DATEFICHE=:NEW.DATEFICHE-1 AND PAT_IDPAT=NUMPAT)
	IF :NEW.RESULTATELECTRO<3 THEN

	
		IF RES <3 THEN
			ELECTRO_VAL=ELECTRO_VAL.NEXTVAL;
		ELSE
		
		END IF;
		IF ELECTRO_VAL>=2 THEN
			UPDATE PATIENT SET DATE_ANA=DATE_ANA+1 WHERE NUMPAT=IDPATIENT;
		ELSE
			IF RISQUE=1 THEN
				
			ELSE
				UPDATE PATIENT SET DATE_ANA=DATE_ANA+5 WHERE NUMPAT=IDPATIENT;
		END IF;
	
END;

--PATIENT
	-- TRIGGER QUI VERIFIE QUE L'AGE DU PATIENT SE TROUVE BIEN ENTRE 25 ET 60
...
CREATE TRIGGER VAL_ELECTRO
BEFORE INSERT OR UPDATE ON ELECTROPENCEPHALOGRAMME
BEGIN
	IF NEW.RESULTATELECTRO > 10 OR NEW.RESULTATELECTRO < 0 THEN
		RAISE_APPLICATION_ERROR(-20001,'Valeur electro comprise entre 0 et 10 seulement');
	END IF;
END;


----------------------------------------------
CREATE TRIGGER TRI_MAJ
...
INSERT INTO PATIENT(NOMP,PRENOMP,AGEP,SEXE,NUMEROCHAMBRE,EXCLU) VALUES ('toto','tata',30,1,25,1);
-----------------------------------
alter table Patient alter column SEXE VARCHAR2(1 BYTE)
-------------------------------
...
BEGIN 
    ...
    SELECT PAT_IDPAT INTO NUMPAT FROM FICHE WHERE :NEW.ANA_ID_FICHE = IDFICHE;   
    SELECT DATEFICHE INTO DATETRAIT FROM FICHE WHERE :NEW.ANA_ID_FICHE=IDFICHE;
    SELECT TRUNC((SYSDATE - DATE_ARRI)+1) INTO NUMJOUR FROM PATIENT WHERE IDPATIENT=NUMPAT;
	IF :NEW.PLAQUETTES>400000 OR :NEW.PLAQUETTES<150000 THEN
		HORSNORME:=HORSNORME+1;
	END IF;
	IF :NEW.POTASSIUM>5 OR :NEW.POTASSIUM<3.5 THEN
		HORSNORME:=HORSNORME+1;
	END IF;
...
END;
------------------------
--Permet de mettre à jour le planning du patient lors de la saisie d'une fiche 
--(ex : une analyse de sang sera plannifiée 5 jours après la précédente si l'analyse n'est pas à refaire). 

CREATE OR REPLACE TRIGGER TOTO
BEFORE INSERT OR UPDATE ON FICHE
FOR EACH ROW
BEGIN
	SELECT RESULTATELECTRO INTO RESPRECEDENT FROM ELECTROPENCEPHALOGRAMME WHERE(SELECT DATAFICHE FROM FICHE WHERE )
------------------------------------------------------------
-----------------------------------------
CREATE OR REPLACE TRIGGER ELECTRO_AUTO_METIER
AFTER INSERT ON ELECTROENCEPHALOGRAMME
